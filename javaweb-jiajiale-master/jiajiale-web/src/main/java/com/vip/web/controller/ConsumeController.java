...
public class ConsumeController implements StringUtil {
	...
	public JSONResponse list(QueryVipConsumeAo ao , String pageNo,String pageSize )throws  Exception{
		Integer pageNoInt = parseInt(pageNo,1);
		Integer pageSizeInt =parseInt(pageSize,10);
		QueryResult<VipConsumeEntity> vipConsumeEntityQueryResult = service.queryConsume(ao, pageNoInt, pageSizeInt);
		QueryResult<VipConsumeListVO> queryVoResult = vipConsumeEntityQueryResult.rowsMap(VipConsumeListVO::of);
		return JSONResponse.ok(queryVoResult);
	}
}
